// Hutchens lab 
//modified in November 2022 to allow mapping of one channel and quantifying of another, in case the quantifying channel is faint.
//modified to take a set mean and sd as arguments (this will be the precalculated mean for a whole group)
//and provide outcomes based on that mean and SD as well as the mean and SD for the single image evaluated here.
...
GROUP_MEAN=100;  //need to be mapped to 8 bit
GROUP_SD=100;
...
COUNTING_FRAME_CROSSHAIR_COUNT = 25; // Points per ROI - the actual number used will be a square number based on the square root of this number
...
if(lengthOf(arguments) > 0) {
	...
	COLOR_DECONVOLVE_BLUE		= (parseInt(argarray[5]) == 1);
	ENHANCE_IMAGE				= (parseInt(argarray[6]) == 1);
	CHANNEL_TO_USE 				=parseInt(argarray[7]);
	CHANNEL_TO_THRESH			=parseInt(argarray[8]);
	SAVE_ROI_OVERLAY			= (parseInt(argarray[9]) == 1);
	DEFAULT_SCALE 				= parseFloat(argarray[10]);
	GRID_WIDTH_UM 				= parseFloat(argarray[11]);
	GRID_HEIGHT_UM 				= parseFloat(argarray[12]);
	FRAME_BOX_WIDTH_UM 			= parseFloat(argarray[13]);
	FRAME_BOX_HEIGHT_UM 		= parseFloat(argarray[14]);
	threshold_step_1[0]			= parseFloat(argarray[15]);
	threshold_step_1[1]			= parseFloat(argarray[16]);
	threshold_step_2[0]			= parseFloat(argarray[17]);
	threshold_step_2[1]			= parseFloat(argarray[18]);
	FRACTION_TO_SAMPLE 			= parseFloat(argarray[19]);
	ESTIMATE_AREA_SAMPLED		= (parseInt(argarray[20]) == 1);
	COUNTING_FRAME_CROSSHAIR_COUNT = parseFloat(argarray[21]);
	SD_THRESHOLD_POSITIVE_MIN 		= parseFloat(argarray[22]);
	SD_THRESHOLD_POSITIVE_MAX 		= parseFloat(argarray[23]);
	GROUP_MEAN					=parseInt(argarray[24]);
	GROUP_SD					=parseInt(argarray[25]);
	TARGET_DIRECTORY			= argarray[26];
...
run("Split Channels");
namestr="C"+CHANNEL_TO_THRESH+"-"+original_image_name;
selectWindow(namestr);
image_to_map=namestr;
image_to_quantify="C"+CHANNEL_TO_USE+"-"+original_image_name;
tissue_map_window_title = '';
results = prepare_without_injurymap(image_to_map);
KIDNEY_COMPUTED_AREA = results[0];
tissue_map_window_title = results[1];
selectWindow(image_to_quantify);

	setBatchMode (false); // duplicate does not work correctly in batch mode
	selectWindow(image_to_quantify);
	run("Duplicate...", "title=ORIGINAL_TO_QUANTIFY");
	selectWindow("ORIGINAL_TO_QUANTIFY");
...
if(!NO_USER_INPUT)
	waitForUser (tissue_map_window_title);
...
RANDOM_START = (random*ROI_INTERVAL);
	...
	if (COLOR_DECONVOLVE_BLUE==true){  //do the color deconvolution and close all but the blue image before saving the tile image
		...
		close("ORIGINAL_TO_QUANTIFY");
		selectWindow(conv_image+"-(Colour_2)");
		rename("ORIGINAL_TO_QUANTIFY");
		run("8-bit");
...
selectWindow("ORIGINAL_TO_QUANTIFY");
myEnhanceImage("ORIGINAL_TO_QUANTIFY");
...
function prepare_without_injurymap(window_name_for_mask) {
	...
	selectWindow(window_name_for_mask);
	...
	setBatchMode(false); // duplicate does not work correctly in batch modes
	run("Duplicate...", "title=TISSUE_MASK_TO_SAVE");	
	selectWindow("TISSUE_MASK_TO_SAVE");
	setBatchMode (USE_BATCH_MODE);
...
function savestats(pathtofilename,filename){
	...
	//also, the way this is written is not very beautiful.
	...
	//uses globals GROUP_MEAN and GROUP_SD
	...
	for (a=0; a<number_stats; a++) {
		...
		*/
		...
		// count all the outcomes relative to the GROUP_MEAN and GROUP_SD	
	//but need to map those values to 8bit from 16 bit nah, give the script 8 bit
		//gm8= map16to8 (GROUP_MEAN);
		//gs8= map16to8 (GROUP_SD);
		distance_from_group_mean = 0;
		if(ASSUME_LIGHT_BACKGROUND) {
			distance_from_group_mean = GROUP_MEAN - res_stats;
		}
		else {
			distance_from_group_mean = res_stats - GROUP_MEAN;
		}
		if (distance_from_group_mean>0) nggm++; 
		if (distance_from_group_mean>(GROUP_SD*0.25)) nggqsd++;
		if (distance_from_group_mean>(GROUP_SD*0.5)) ngghsd++;
		if (distance_from_group_mean>(GROUP_SD*1)) ngg1sd++;
		if (distance_from_group_mean>(GROUP_SD*2)) ngg2sd++;
		if (distance_from_group_mean>(GROUP_SD*3)) ngg3sd++;
	...
	//add min, max, SD to line below (also z score?)
	...
	stats_data = newArray(original_image_name,	number_stats,		ntissue,			ngt,						npos,				npos/ntissue,	ngm, ngqs,nghs,				ngs,			ng2s,	ng25s,	ng3s,		SD_THRESHOLD_POSITIVE_MIN,	SD_THRESHOLD_POSITIVE_MAX,	mean_stats,	SD_stats,	CV_stats,origIMG_AREA, origIMG_MEAN, origIMG_MIN, origIMG_MAX, origIMG_STD, GROUP_MEAN, GROUP_SD, nggm, nggqsd, ngghsd, ngg1sd, ngg2sd, ngg3sd);
...
function beginStatsFile(main_directory) {
	...
	print(statsfile, "image,points,points on tissue,points greater than min, points positive,pos/tissue,>mean,Z> 0.25 SD,Z> 0.5 SD,Z > 1 SD,Z > 2 SD,Z > 2.5 SD,Z > 3 SD,Z threshold min,Z threshold max, mean,sd,cv,origArea,origMean,origMin,origMax,origSTD,gm8,gs8, nggm,nggqsd,ngghsd,ngg1sd,ngg2sd,ngg3sd\r\n");
...
function recordParametersToFile(filename) {
	...
	parameters_log_text += "\r\nCHANEL_TO_USE (QUANT):          "+CHANNEL_TO_USE;
	parameters_log_text += "\r\CHANNEL_TO_THRESH:          "+CHANNEL_TO_THRESH;
...
function myEnhanceImage(windowname){
 ...
 function gettime() {
  ...
  function map16to8(value){
//slope = (output_end - output_start) / (input_end - input_start)
//output = output_start + slope * (input - input_start)
  	fxn_slope = 0.00389105058; //the constant relationship between 8 and 16 bit scales (255/65535)
  	return ((0+fxn_slope*(value-0));
  }
