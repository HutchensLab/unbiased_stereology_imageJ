//This macro is designed to start with a scanned slide (the axioscan uses a .czi file holding multiple images)
//then open the czi, select only the images of desired size (very large images exceed the processing capability of some computers for later steps)
//and for each single series (which is an image, not a series of images), run the stereology macro to perform image quantification
//this file combines code from my czi_opener test and from Nick McClellan's morphometry_batch macro.
...
// these variables should match the ones present in the image procesing script
// and match those used in the parameters dialog
...
CHANNEL_TO_USE = "green"; // options are all, grayscale, red, green, blue
...
// Get directories  and desired series x size
...
if (format=="Zeiss CZI"){
	...
	for (ser=0; ser<lengthOf(series_values); ser++) {
		processCZISeries(path,filename_prefix,series_values[ser]+1,arguments);
